...
class PathSelectorFrame(tk.Frame):
    def __init__(self, master, controller):
        ...
        tk.Button(self, text="에셋 폴더 선택", command=self.select_asset).pack(
            side="left", padx=5
        )
        tk.Label(self, textvariable=self.asset_path_var, width=60, anchor="w").pack(
            side="left"
        )

        tk.Button(self, text="모드 폴더 선택", command=self.select_mod).pack(
            side="left", padx=5
        )
        tk.Label(self, textvariable=self.mod_path_var, width=60, anchor="w").pack(
            side="left"
        )
